 ...
 * <LinearLayout
            ...
            app:layout_behavior="com.eposeidon.lib_ui.behavior.DrawerBottomSheetBehavior">
    </LinearLayout>
 */
public class DrawerBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    public DrawerBottomSheetBehavior() {
        this.dragCallback = new NamelessClass_1();
    }

    public DrawerBottomSheetBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.dragCallback = new NamelessClass_1();
    ...
    class NamelessClass_1 extends ViewDragHelper.Callback {
        ...
        public boolean tryCaptureView(@NonNull View child, int pointerId) {
            if (DrawerBottomSheetBehavior.this.state == STATE_DRAGGING) {
                return false;
            } else if (DrawerBottomSheetBehavior.this.touchingScrollingChild) {
                return false;
            } else {
                if (DrawerBottomSheetBehavior.this.state == 3 && DrawerBottomSheetBehavior.this.activePointerId == pointerId) {
                    View scroll = (View) DrawerBottomSheetBehavior.this.nestedScrollingChildRef.get();
                    if (scroll != null && scroll.canScrollVertically(-1)) {
                        return false;
                    }
                }

                return DrawerBottomSheetBehavior.this.viewRef != null && DrawerBottomSheetBehavior.this.viewRef.get() == child;
            }
        }

        @Override
        public void onViewPositionChanged(@NonNull View changedView, int left, int top, int dx, int dy) {
            DrawerBottomSheetBehavior.this.dispatchOnSlide(top);
        }

        @Override
        public void onViewDragStateChanged(int state) {
            if (state == 1) {
                DrawerBottomSheetBehavior.this.setStateInternal(STATE_DRAGGING);
            }
        ...
        public void onViewReleased(@NonNull View releasedChild, float xvel, float yvel) {
            ...
            int currentTop;
            if (yvel < 0.0F) {
                if (DrawerBottomSheetBehavior.this.fitToContents) {
                    currentTop = releasedChild.getTop();

                    if (currentTop < (collapsedOffset + HIDE_THRESHOLD) && currentTop >= halfExpandedOffset) {
                        top = DrawerBottomSheetBehavior.this.halfExpandedOffset;
                        targetState = STATE_HALF_EXPANDED;
                    } else {
                        top = DrawerBottomSheetBehavior.this.fitToContentsOffset;
                        targetState = STATE_EXPANDED;
                    }

                } else {
                    currentTop = releasedChild.getTop();
                    if (currentTop > DrawerBottomSheetBehavior.this.halfExpandedOffset) {
                        top = DrawerBottomSheetBehavior.this.halfExpandedOffset;
                        targetState = STATE_HALF_EXPANDED;
                    } else {
                        top = 0;
            ...
            } else if (!DrawerBottomSheetBehavior.this.hideable || !DrawerBottomSheetBehavior.this.shouldHide(releasedChild, yvel) || releasedChild.getTop() <= DrawerBottomSheetBehavior.this.collapsedOffset && Math.abs(xvel) >= Math.abs(yvel)) {
                if (yvel != 0.0F && Math.abs(xvel) <= Math.abs(yvel)) {
                    ...
                    if (currentTop < halfExpandedOffset) {
                        ...
                        top = DrawerBottomSheetBehavior.this.collapsedOffset;
                    ...
                    currentTop = releasedChild.getTop();
                    if (DrawerBottomSheetBehavior.this.fitToContents) {
                        if (Math.abs(currentTop - DrawerBottomSheetBehavior.this.fitToContentsOffset) < Math.abs(currentTop - DrawerBottomSheetBehavior.this.collapsedOffset)) {
                            top = DrawerBottomSheetBehavior.this.fitToContentsOffset;
                            targetState = STATE_EXPANDED;
                        } else {
                            top = DrawerBottomSheetBehavior.this.collapsedOffset;
                    ...
                    } else if (currentTop < DrawerBottomSheetBehavior.this.halfExpandedOffset) {
                        if (currentTop < Math.abs(currentTop - DrawerBottomSheetBehavior.this.collapsedOffset)) {
                            top = 0;
                            targetState = STATE_EXPANDED;
                        } else {
                            top = DrawerBottomSheetBehavior.this.halfExpandedOffset;
                    ...
                    } else if (Math.abs(currentTop - DrawerBottomSheetBehavior.this.halfExpandedOffset) < Math.abs(currentTop - DrawerBottomSheetBehavior.this.collapsedOffset)) {
                        top = DrawerBottomSheetBehavior.this.halfExpandedOffset;
                        targetState = STATE_HALF_EXPANDED;
                    } else {
                        top = DrawerBottomSheetBehavior.this.collapsedOffset;
                ...
                top = DrawerBottomSheetBehavior.this.parentHeight;
                targetState = STATE_HIDDEN;
            }

            if (DrawerBottomSheetBehavior.this.viewDragHelper.settleCapturedViewAt(releasedChild.getLeft(), top)) {
                DrawerBottomSheetBehavior.this.setStateInternal(STATE_SETTLING);
                ViewCompat.postOnAnimation(releasedChild, DrawerBottomSheetBehavior.this.new SettleRunnable(releasedChild, targetState));
            } else {
                DrawerBottomSheetBehavior.this.setStateInternal(targetState);
            }
        ...
        public int clampViewPositionVertical(@NonNull View child, int top, int dy) {
            return MathUtils.clamp(top, DrawerBottomSheetBehavior.this.getExpandedOffset(), DrawerBottomSheetBehavior.this.hideable ? DrawerBottomSheetBehavior.this.parentHeight : DrawerBottomSheetBehavior.this.collapsedOffset);
        }
        ...
        public int getViewVerticalDragRange(@NonNull View child) {
            return DrawerBottomSheetBehavior.this.hideable ? DrawerBottomSheetBehavior.this.parentHeight : DrawerBottomSheetBehavior.this.collapsedOffset;
        }
    }
                ...
                if (child != null) {
                    ...
                    if (parent != null && parent.isLayoutRequested() && ViewCompat.isAttachedToWindow(child)) {
                        child.post(new Runnable() {
                            @Override
                            public void run() {
                                DrawerBottomSheetBehavior.this.startSettlingAnimation(child, state);
                            }
                        });
                    } else {
    ...
    public static <V extends View> DrawerBottomSheetBehavior<V> from(V view) {
        ViewGroup.LayoutParams params = view.getLayoutParams();
        if (!(params instanceof CoordinatorLayout.LayoutParams)) {
            throw new IllegalArgumentException("The view is not a child of CoordinatorLayout");
        } else {
            CoordinatorLayout.Behavior behavior = ((CoordinatorLayout.LayoutParams) params).getBehavior();
            if (!(behavior instanceof DrawerBottomSheetBehavior)) {
                throw new IllegalArgumentException("The view is not associated with BottomSheetBehavior");
            } else {
                return (DrawerBottomSheetBehavior) behavior;
            }
        }
    }
    ...
    private class SettleRunnable implements Runnable {
        ...
        @Override
        public void run() {
            if (DrawerBottomSheetBehavior.this.viewDragHelper != null && DrawerBottomSheetBehavior.this.viewDragHelper.continueSettling(true)) {
                ViewCompat.postOnAnimation(this.view, this);
            } else {
                DrawerBottomSheetBehavior.this.setStateInternal(this.targetState);
            }
